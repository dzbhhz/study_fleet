...
for j in range(len(lon_i)):
    for i in range(len(lat_i)):
        LON.append(round(lon_i[j],1))
        NUMBER.append(number[j][i])
for i in range(len(lon_i)):
    for j in range(len(lat_i)):
        LAT.append(round(lat_i[j],1))                    #use for griddata
number_i = griddata(np.array(LON),np.array(LAT),np.array(NUMBER),lon_is,lat_is)
depth_i = griddata(np.array(lon),np.array(lat),np.array(data['MEAN_DEPTH']),lon_is,lat_is)
...
CS = plt.contourf(lon_is, lat_is, number_i, np.arange(0,200,5), cmap=plt.cm.rainbow,
                  vmax=abs(number_i).max(), vmin=-abs(number_i).max())
