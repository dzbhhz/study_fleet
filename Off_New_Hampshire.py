...
temp_fvcom=pd.Series(data['fvcom_temp'])
...
for i in range(len(lat)):
    ...
    if n==370 and m==245:        #site off New Hampshire 
        indx.append(i)

time=[2009,2011,2012]
month=[[11,5,17],[6,4,116],[6,18,30]]     #set three time period
INDX_month=[[],[],[]]
...
plt.plot(data['datet'][indx].values,data['fvcom_temp'][indx].values,label='mod')
plt.legend(loc='best')
...
ax1.plot(data['datet'][INDX_month[0]].values,temp_fvcom[INDX_month[0]],linestyle=':',label='mod',linewidth=5,c='black')
ax1.grid(True)
ax2=fig.add_subplot(312)
ax2.plot(data['datet'][INDX_month[1]].values,data['MEAN_TEMP'][INDX_month[1]].values,linewidth=5,c='black')
ax2.plot(data['datet'][INDX_month[1]].values,temp_fvcom[INDX_month[1]],linestyle=':',linewidth=5,c='black')
ax2.grid(True)
ax3=fig.add_subplot(313)
ax3.plot(data['datet'][INDX_month[2]].values,data['MEAN_TEMP'][INDX_month[2]].values,linewidth=5,c='black')
ax3.plot(data['datet'][INDX_month[2]].values,temp_fvcom[INDX_month[2]],linestyle=':',linewidth=5,c='black')
ax3.grid(True)
ax1.legend(loc='best')
