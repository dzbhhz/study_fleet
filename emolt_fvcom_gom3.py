...
if method=='Wave-height':
    ...
    try:
        ...
        itime=np.argmin(abs(np.array(T)-start))   #find nearest time
        daystr=T[itime]
        BIN= [alon-10*one_minute,alon+10*one_minute,alat-10*one_minute,alat+10*one_minute] # box you want to plot,here 20>5 want to remove boundaries in ax.
        indx=argwhere((lons >= BIN[0]) & (lons <= BIN[1]) & (lats >= BIN[2]) & (lats <= BIN[3]))  # find  points in bounding box
        INDX=[]    
        for i in range(len(indx)):
            INDX.append(indx[i][0])
        ...
        plt.title('Wave height,'+str(daystr)[0:-6]+'(local time)',fontsize=15)
        plt.savefig('/var/www/html/ioos/sf/fig/'+method+'.png')
    except:
        nc1='Model does`t work now'
'''
